<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.name" placeholder="请输入标签名称" style="width: 200px;" class="filter-item" clearable @change="handleFilter"/>
      <el-select v-model="listQuery.status" placeholder="请选择状态" clearable style="width: 200px" class="filter-item" @change="handleFilter">
        <el-option v-for="(item,index) in statusList" :key="index" :label="item" value="0"></el-option>
      </el-select>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="300px" align="center" label="标签名称">
      ...
      <el-table-column label="排序" align="center" prop="sort" width="80" sortable>
        <template slot-scope="{row}">
          <el-input v-model="row.sort" @blur="labelSorts(row)"></el-input>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="150px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-waves type="primary" size="mini" @click="labelEdit(row.id)">
            编辑
          </el-button>
          <el-button v-waves size="mini" type="danger" @click="labelDestroy(row.id)">
            删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog   :title="title" :visible.sync="dialogVisible" width="80%" @close="dialogClose">
      ...
      <el-form label-width="100px" :model="form" :rules="rules" ref="ref">
        <el-form-item label="标签名称" prop="name">
          <el-input placeholder="请输入标签名称" maxlength="30" clearable show-word-limit v-model="form.name"
          ></el-input>
        </el-form-item>
...
<script>
import { labelIndex,labelStatus,labelStore,labelEdit,labelUpdate,labelSorts,labelDestroy } from '@/api/blog/label'
export default {
  name: 'LabelIndex',
  data() {
  ...
  methods: {
    ...
    async getList() {
      ...
      labelIndex(this.listQuery).then(response => {
    ...
    primary() {
      this.$refs.ref.validate(valid => {
        if (valid) {
          if(this.form.id){
            labelUpdate(this.form).then(response => {
              if(response.status === 20000){
            ...
            labelStore(this.form).then(response => {
              if(response.status === 20000){
                this.$base.message({ message: response.message })
                this.dialogVisible = false
                this.getList()
    ...
    // 状态调整
    labelStatus(info) {
      labelStatus({id:info.id,status:info.status}).then(response => {
        if(response.status === 20000){
          this.$base.message({message:response.message})
        }else{
    ...
    labelDestroy(id) {
      this.$base.confirm(
        { content: "确定要删除该项吗！" },
        () => {
          labelDestroy({id:id}).then(response => {
            if(response.status === 20000){
              this.$base.message({ message: response.message })
              this.getList()
            }
    ...
    labelSorts(info){
      labelSorts({id:info.id,sort:info.sort}).then(response => {
        if(response.status === 20000){
          this.$base.message({message:response.message})
        }else{
          this.getList()
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['编号', '标签名称','状态','创建时间']
        const filterVal = ['id', 'name','status','created_at']
